...
def check_fill_before_sign(rule, entry) -> List[ValidationError]:
    ...
    if entry["Teilnahmeerklärung versendet"] == "fehlt":
        errors.append(
            ValidationError(rule, entry, f"Teilnahmeerklärung versendet", "", "", "Teilnahmeerklärung Selektivvertrag fehlt. Diese muss in die Ablage."),
        )
    return errors
